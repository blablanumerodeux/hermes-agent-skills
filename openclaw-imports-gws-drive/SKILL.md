...
```bash
# Get file metadata first (includes name, size, mimeType)
gws drive files get --params '{"fields": "name,mimeType,size"}' <fileId>

# Download binary content — REQUIRED: alt=media in params
gws drive files get <fileId> \
  --params '{"alt": "media"}' \
  --output /tmp/output.pdf
```

> **Note:** Without `alt=media`, the API returns JSON metadata only — no file content.

### Search for files

```bash
# List files (or use q= to filter)
gws drive files list --params '{"pageSize": 10, "orderBy": "modifiedTime desc"}'

# Search by name
gws drive files list --params '{"q": "name contains '\''permit'\''"}'

# Search by MIME type
gws drive files list --params '{"q": "mimeType contains '\''pdf'\''"}'

# Combined search
gws drive files list --params '{"q": "name contains '\''driver'\'' and mimeType contains '\''pdf'\''"}'
```

### Upload a file

See `+upload` helper skill.
