...
## Environment & Auth (this setup)

On this system:
- **Binary path:** `/home/linuxbrew/.linuxbrew/Cellar/googleworkspace-cli/0.22.5/bin/gws` (linuxbrew, not in standard PATH)
- **Credentials:** stored in `~/.config/gws/` — token.json, token_cache.json, client_secret.json
- **Auth workaround:** if `gws auth` fails, set `GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE=~/.config/gws/token.json`

Example with env override:
```bash
GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE=~/.config/gws/token.json gws gmail +send --to ana@example.com --subject "Hi" --body "Hey Ana"
```
